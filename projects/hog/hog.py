...
def more_boar(player_score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    if player_score >= 10:
        leftMostPlayer = int(str(player_score)[0])
        secondLeftPlayer = int(str(player_score)[1])
    else:
        leftMostPlayer = 0
        secondLeftPlayer = player_score
    if opponent_score >= 10:
        leftMostOpp = int(str(opponent_score)[0])
        secondLeftOpp = int(str(opponent_score)[1])
    else:
        leftMostOpp = 0
        secondLeftOpp = opponent_score
    return leftMostPlayer < leftMostOpp and secondLeftPlayer < secondLeftOpp
    # END PROBLEM 4
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    announcement = "Player {} has reached a new maximum point gain. {} point(s)!"
    def announce(score0, score1):
        highest_gains = running_high
        if who == 0:
            score = score0
        else:
            score = score1
        gains = score - last_score
        if gains > highest_gains:
            print(announcement.format(who, score - last_score))
            highest_gains = gains
        return announce_highest(who, score, highest_gains)
    return announce
    # END PROBLEM 7
